...
kubectl apply -f k8s
kubectl set image deployments/server-deployments server=mike2help/multi-server:$SHA
kubectl set image deployments/client-deployments client=mike2help/multi-client:$SHA
kubectl set image deployments/worker-deployments worker=mike2help/multi-worker:$SHA
